        ...
        <c:forEach var="type" items="<%=SectionType.values()%>">
            ...
            <c:choose>
                ...
                <c:when test="${type=='PERSONAL'}">
                    <textarea name="${type}" id="" cols="75" rows="5"><%=section%></textarea>
                </c:when>
                <c:when test="${type=='PERSONAL'}">
                    <textarea name="${type}" id="" cols="75" rows="5"><%=section%></textarea>
                </c:when>
                <c:when test="${type=='ACHIEVEMENT'|| type=='QUALIFICATIONS'}">
                    <textarea name="${type}" id="" cols="75" rows="5"><%=String.join("\n", ((ListSection)section).getInfoList())%></textarea>
                </c:when>
                        ...
                        <c:forEach var="pos" items="${org.positions}">
                            ...
                            <dl>
                                ...
                                <dd><input type="text" name="${type}${counter.index}startDate" size="10"
                                           value="<%=DateUtil.dateForJsp(pos.getStartDate())%>" placeholder="MM/YYYY"></dd>
                            </dl>
                            <dl>
                                <dt>Конечная дата:</dt>
                                <dd><input type="text" name="${type}${counter.index}endDate" size="10"
                                           value="<%=DateUtil.dateForJsp(pos.getEndDate())%>" placeholder="MM/YYYY"></dd>
                            </dl>
                            <dl>
                                <dt>Должность:</dt>
                                <dd><input type="text" name="${type}${counter.index}title" size="100" value="${pos.title}"></dd>
                            </dl>
                            <dl>
                                <dt>Описание:</dt>
                                <dd><input type="text" name="${type}${counter.index}description" size="100" value="${pos.description}"></dd>
                            </dl>
